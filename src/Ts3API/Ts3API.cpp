...
bool Ts3API::trystoi(string str, int& returnedValue)
{
	...
	{
		cout << "Konwersja ciągu znaków na liczbę nie powiodła się.";
		return false;
	}
}
	...
	else
	{
		if ((pos = input.find("|")) != string::npos)
		{
			...
			if (bufferMap[mapIndex] == "") {
				cout << "Próbowano zwinąć mapę z nieistniejącym lub pustym indeksem. index: " + mapIndex;
				input.erase(input.length() -1, 1);
				return;
			}
...
{
	int timestampint;
	if(!trystoi(timestamp, timestampint)) cout << "Nie udało się zamienić typu string na int. convertTimetampTime()";
